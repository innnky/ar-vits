...
class TTSDecoder(nn.Module):
    ...
    def forward(self, q, phone, q_mask, phone_mask):
        ...
        audio_output = self.transducer.decode(audio_output, q_input).squeeze(2)
...
#                            print (n_seats, stopping_streams_idx, stopped.size(), stop_seats)
        ...
        final_outputs = final_outputs[0].squeeze(1)
        final_outputs = torch.cat([pri.squeeze(0), final_outputs])
        if output_alignment:
